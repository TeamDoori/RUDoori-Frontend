...
const MyboardDetail = ({ navigation, route }) => {
  ...
  const comments = useSelector((state) => {
    if (pageInfo.toString() === "Myscrap") {
      return state.board.scrapCommentList;
    } else if (pageInfo.toString() === "Myboard") {
      return state.board.myBoardCommentList;
    }
    return [];
  });
  ...
  const _handleDeletePostButtonPress = () => {
    ...
    if (pageInfo.toString() === "Myboard") {
      navigation.navigate("Myboard");
    } else if (pageInfo.toString() === "Myscrap") {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("EditMyboardDetail", { post, pageInfo })
        }
      >
        <Text>[ 수정 ]</Text>
      </TouchableOpacity>
